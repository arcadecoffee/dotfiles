...
if [ "$color_prompt" = yes ]; then
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[33m\]$(git branch 2> /dev/null | sed -e "/^[^*]/d" -e "s/* \(.*\)/ (\1)/")\[\033[00m\] \$ '
else
...
HEROKU_AC_BASH_SETUP_PATH=/Users/jgregoire/Library/Caches/heroku/autocomplete/bash_setup && test -f $HEROKU_AC_BASH_SETUP_PATH && source $HEROKU_AC_BASH_SETUP_PATH;


# some helper functions for working with Docker
docker-log () { docker logs -f $(docker ps -qf name=$1); }
docker-shell () { docker exec -it $(docker ps -qf name=$1) /bin/bash; }
docker-kill () { docker kill $(docker ps -qf name=$1) /bin/bash; }
